...
PvPの中継はクライアントの送信をそのまま相手へ渡すが，元サーバーが補完していた値 (コスト・条件回答) が抜ける．そこでクライアントのバトルエンジンをヘッドレスで並走させ，同じ試合を再生して抜けた値を観測する．今は観測のみで，裁定への切り替えは環境変数`OPENVERSE_ENGINE_ROLE`で段階的に有効化する．
