...
- VPN (ポート開放が不要で身内向き)
- 固定ホスト (ポート開放やDDNSが必要)
- HTTPS固定だが，クライアントは証明書を検証しないので自己署名証明書をそのまま使える (クライアント改変やmkcert不要，証明書はランチャーが生成)

## バトルエンジン

PvPの中継はクライアントの送信をそのまま相手へ渡すが，元サーバーが補完していた値 (コスト・条件回答・`spin`) が抜ける．そこでクライアントのバトルエンジンをヘッドレスで並走させ，同じ試合を再生して抜けた値を読む．

エンジンは部屋ごとに2基持つ．クライアントは受信メッセージを表示するのではなく再シミュレートするので，各クライアントの視界を個別にモデル化しないと`spin`も受信検査も出せない．

信用の度合いは`OPENVERSE_ENGINE_ROLE`で段階的に上げる (`Observe` → `AdviseCost` → `AnswerBlanks` → `DecideResult`)．既定は`AdviseCost`で，`DecideResult`は未実装．原因の調査結果と残る限界は[desync.md](desync.md)にある．

## 配布

- ランチャーとセットアップの2つのexeで配る (setupは昇格不要，launcherが管理者権限でhosts書き換えとサーバー起動を行う)
- 自己ホストは`dotnet run`でも動く (SQLiteなのでDBサーバー不要)
- 常時稼働が要るならDocker化も視野
